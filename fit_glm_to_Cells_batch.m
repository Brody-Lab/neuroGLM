function res = fit_glm_to_Cells_batch(rat,varargin)
    ...
    for i=1:length(possible_sessions)
        ...
        if ~any(file_contents.Msorted.n_cluster) && ~any(file_contents.Cells.n_cluster)
            res{i} = 'no cells';
            fprintf([res{i},'\n']);
            continue
        end
        try
            try
                ...
                file_contents.Msorted = PB_make_PETH(file_contents.Msorted);
                [a,b,c] = fileparts(msorted_filename);
                fit_glm_to_Cells(file_contents.Msorted,'use_parallel',true,'save',true,varargin{:},'save_path',[a,filesep,b,'_glmfits.mat']);
                res{i} = 'success';
            catch
                ...
                [a,b,c] = fileparts(msorted_filename);
                fit_glm_to_Cells(file_contents.Cells,'use_parallel',true,'save',true,varargin{:},'save_path',[a,filesep,b,'_glmfits.mat']);
                res{i} = 'success';                
            end
        catch ME
            res{i} = ME;
